...
void DynamicSurfaceManagerKinect::gcSurfaces() {
        ...
        for (int i = 0 ; i < MAX_DYNAMIC_SURFACES ; i++) {
                ...
                if ((label_map[i]!=-1 && (label_map_lastused[i] == 0 || label_map_lastused[i] < threshold)) || (label_map[i]==-2)) { //label_map_lastused[i]>0 && label_map_lastused[i] < threshold)) {
                        ...
                        vector<ofVec3f> hidden;
                        for (int x = 0 ; x < MAX_VERTEX ; x++)
                                hidden.push_back(ofVec3f(0,0));//,ofVec{$1}(0,0),ofVec{$1}(0,0),ofVec{$1}(0,0)};
			hidden.push_back(ofVec3f(0,0));
			try {
...
void DynamicSurfaceManagerKinect::update() {
	...
	//int i = 0;
		...
		//ofVec{$1} old = piMapper->getApp().getSurfaceManager()->getSurface(index)->getVertices();
		...
		cout << "got hex with points size " << points.size() << endl;
			...
			//projectedPoint = ofVec{$1}(points[j].x,points[j].y);	
		    	stroke.push_back(projectedPoint * ofVec2f(1024,768)); //ofVec{$1}(projectedPoint.x,projectedPoint.y));
		}
		stroke.push_back(
		...
		//stroke.push_back(ofVec{$1}(points[0].x,points[0].y)*ofVec{$1}(1024,768));
